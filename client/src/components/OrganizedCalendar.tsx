...
export default function OrganizedCalendar({
  batches,
  products,
  workshops,
  onBatchClick,
  viewType,
}: OrganizedCalendarProps) {
  ...
  const sortedWorkshops = [...workshops].sort(
    (a, b) => (a.scheduleOrder || 1) - (b.scheduleOrder || 1),
  );
  ...
  const getBatchPosition = (batch: Batch, viewDaysArray: Date[]) => {
    ...
    const startIndex = viewDaysArray.findIndex(
      (day) => day.toDateString() === cutDate.toDateString(),
    );

    const endIndex = viewDaysArray.findIndex(
      (day) => day.toDateString() === returnDate.toDateString(),
    );
  ...
  return (
    <div className="w-full h-full bg-white rounded-lg shadow-lg">
      ...
      >
        {/* Day Headers */}
        <div
          className="grid gap-1 text-center text-sm font-medium text-gray-700 mb-4"
          style={{ gridTemplateColumns: `repeat(${viewDays.length}, 1fr)` }}
        >
          {viewDays.map((day, index) => {
            ...
            return (
              <div key={index} className="p-2 relative">
                ...
                <div
                  className={`font-semibold relative z-10 ${isToday ? "text-orange-800" : ""}`}
                >
                  {
                ...
                <div
                  className={`text-xs relative z-10 ${isToday ? "text-orange-700" : "text-gray-500"}`}
                >
                  {formatDate(day)}
                </div>
              </div>
        ...
        <div className="space-y-4">
          {sortedWorkshops.map((workshop, workshopIndex) => {
            const workshopBatches = batches.filter(
              (batch) => batch.workshopId === workshop.id,
            );
            ...
            return (
              <div key={workshop.id} className="relative">
                ...
                <div
                  ...
                  style={{
                    borderBottom:
                      workshopIndex < sortedWorkshops.length - 1
                        ? "0px dotted #d1d5db"
                        : "none",
                    paddingBottom: "0px",
                    marginBottom:
                      workshopIndex < sortedWorkshops.length - 1 ? "0px" : "0",
                ...
                >
                  ...
                  {workshopBatches.map((batch) => {
                    ...
                    const startCol = parseInt(
                      position.gridColumn.split(" ")[0],
                    );
                    const span = position.gridColumn.includes("span")
                      ? parseInt(position.gridColumn.split("span ")[1])
                      : 1;
                    const leftPercent =
                      ((startCol - 1) / viewDays.length) * 100;
                    const widthPercent = (span / viewDays.length) * 100;

                    return (
                      <div
                        key={batch.id}
                        className="absolute top-1/2 transform -translate-y-1/2 rounded-lg p-1 text-white cursor-pointer hover:opacity-90 transition-all duration-200 shadow-sm flex items-center h-[55px] border border-white border-opacity-20"
                        style={{
                      ...
                      >
                        <div className="text-xs font-medium truncate w-full flex items-center">
                          {batch.status === "returned" && (
                            <span className="bg-white text-gray-600 px-1 rounded text-xs mr-1 font-bold">
                              RET
                            </span>
                          )}
                          <span className="truncate">
                            {workshop.name} | LOTE {batch.code} |{" "}
                            {batch.productId
                              ? getProductName(batch.productId).substring(
                                  0,
                                  15,
                                ) +
                                (getProductName(batch.productId).length > 15
                                  ? "..."
                                  : "")
                              : "Múltiplos"}
                          </span>
